...
export default function App() {
  ...
  const endLiveGig = () => {
    if(window.confirm('End this live gig? This will finalize all earnings and ratings.')){
      ...
      const songRatings = liveGigData.ratings.filter(r => r.type === 'song');
      const artistRatings = liveGigData.ratings.filter(r => r.type === 'artist');
      const gigRatings = liveGigData.ratings.filter(r => r.type === 'gig');
      
      const avgSongRating = songRatings.length > 0 
        ? (songRatings.reduce((sum, r) => sum + r.rating, 0) / songRatings.length).toFixed(1)
        : 0;
      const avgArtistRating = artistRatings.length > 0
        ? (artistRatings.reduce((sum, r) => sum + r.rating, 0) / artistRatings.length).toFixed(1)
        : 0;
      const avgGigRating = gigRatings.length > 0
        ? (gigRatings.reduce((sum, r) => sum + r.rating, 0) / gigRatings.length).toFixed(1)
        : 0;
      ...
      setLiveGigData(prev => ({
        ...
        gigEndData: {
          ...
          donationEarnings: prev.donationEarnings,
          requestEarnings: prev.requestEarnings,
          totalDonations: prev.donations.length,
          totalRequests: prev.songRequests.length,
          totalComments: prev.comments.length,
          totalVotes: Object.values(prev.votes).reduce((sum, count) => sum + count, 0),
          avgSongRating,
          avgArtistRating,
          avgGigRating,
          playedSongsCount: prev.playedSongs.length,
          endTime: new Date()
        }
      }));
      
      setMode('art');
      // Keep liveGig and liveGigData for viewing stats
      setTimeout(() => {
        const gigData = storage.get(STORAGE_KEYS.LIVE_GIG_DATA);
        if (gigData?.gigEndData) {
          const stats = gigData.gigEndData;
          window.alert(
            `🎉 Gig Complete!\n\n` +
            `💰 Earnings: ${getCurrencySymbol(liveGigSettings.currency)}${stats.totalEarnings}\n` +
            `🎵 Songs Played: ${stats.playedSongsCount}\n` +
            `💬 Comments: ${stats.totalComments}\n` +
            `⭐ Song Rating: ${stats.avgSongRating}/5\n` +
            `⭐ Artist Rating: ${stats.avgArtistRating}/5\n` +
            `⭐ Gig Rating: ${stats.avgGigRating}/5`
          );
        }
        setLiveGig(null);
      }, 1000);
  ...
  const voteForSong = (songId) => {
    // Check if artist manually disabled voting
    if (!liveGigSettings.acceptingVotes) {
      window.alert('⏸️ Voting is currently closed by the artist.');
      return;
    }
    
    // Check if this is the last song
    if (liveGigData.queuedSongs.length === 1 && liveGigData.currentSong) {
      window.alert('🎵 This is the last song! Voting has closed.');
      return;
    }
    ...
    if(currentVote){
      ...
      setLiveGigData(prev => {
        ...
        const updatedTimestamps = {...prev.voteTimestamps};
        updatedGigVotes[songId] = Math.max(0, (updatedGigVotes[songId] || 1) - 1);
        if (updatedGigVotes[songId] === 0) {
          delete updatedTimestamps[songId];
        }
        return {...prev, votes: updatedGigVotes, voteTimestamps: updatedTimestamps};
      });
    } else {
      ...
      setLiveGigData(prev => {
        ...
        // Record timestamp for first vote
        if (!updatedTimestamps[songId]) {
          updatedTimestamps[songId] = Date.now();
        }
        console.log('🗳️ VOTE ADDED:', {songId, newCount: updatedGigVotes[songId], allVotes: updatedGigVotes});
        return {...prev, votes: updatedGigVotes, voteTimestamps: updatedTimestamps};
      });
    }
    
    setAudienceVotes(newVotes);
    console.log('💾 Vote state updated, triggering sync...');
    // Queue will be updated by useEffect watching votes
  };

  const updateGigQueue = () => {
    if(!liveGig) return;
    
    console.log('🔄 Updating queue...', {votes: liveGigData.votes, limit: liveGigSettings.songLimit});
    
    // Step 1: Get paid requests (ALWAYS HIGHEST PRIORITY)
    const paidRequests = liveGigData.songRequests
      .filter(r => r.status === 'pending')
      .sort((a, b) => b.amount - a.amount)
      .map(r => ({...r.song, isPaid: true, paidAmount: r.amount, priority: 1000 + r.amount}));
    
    // Step 2: Get ALL songs from master playlist that have votes
    const votedSongsFromMaster = masterSongs
      .filter(song => {
        const voteCount = liveGigData.votes?.[song.id] || 0;
        return voteCount > 0 && !liveGigData.playedSongs.includes(song.id);
      })
      .map(song => {
        const voteCount = liveGigData.votes[song.id];
        const timestamp = liveGigData.voteTimestamps?.[song.id] || Date.now();
        return {
          ...song,
          voteCount,
    ...
    // Step 3: Get original gig playlist songs (only those NOT voted OR with votes)
    const gigPlaylist = gigPlaylists.find(p => p.id === liveGig?.playlist);
    const playlistSongs = (gigPlaylist?.songs || [])
      .map(id => {
        const song = masterSongs.find(s => s.id === id);
        if (!song || liveGigData.playedSongs.includes(song.id)) return null;
        ...
        return {
          ...
          voteTimestamp: timestamp,
          priority: voteCount,
          fromOriginalPlaylist: true
        };
      })
      .filter(Boolean);
    
    // Step 4: Combine all songs (paid, voted from master, playlist songs)
    const allSongs = [...paidRequests, ...votedSongsFromMaster, ...playlistSongs];
    
    // Step 5: Remove duplicates (keep highest priority version)
    const uniqueSongsMap = new Map();
    allSongs.forEach(song => {
      const existing = uniqueSongsMap.get(song.id);
      if (!existing || song.priority > existing.priority) {
    ...
    // - Paid requests first (sorted by amount)
    // - Then by vote count (descending)
    // - Ties broken by timestamp (earlier votes first)
    // - Finally by original playlist order
    uniqueSongs.sort((a, b) => {
      // Paid requests always first
      if (a.isPaid && !b.isPaid) return -1;
      if (!a.isPaid && b.isPaid) return 1;
      if (a.isPaid && b.isPaid) return b.paidAmount - a.paidAmount;
      ...
      // Tie-breaker: earlier votes first
      if (a.voteCount > 0 && b.voteCount > 0) {
        return a.voteTimestamp - b.voteTimestamp;
      }
      
      // No votes: original playlist order maintained
      return 0;
    });
    
    // Step 7: Limit to songLimit (removes least priority songs)
    const limitedQueue = uniqueSongs.slice(0, liveGigSettings.songLimit);
    
    // Step 8: Check if this is the last song (close voting automatically)
    const isLastSong = limitedQueue.length === 1 && liveGigData.currentSong;
    
    // Voting closes ONLY when:
    // 1. Artist manually closes it (acceptingVotes = false), OR
    // 2. Last song is playing
    const shouldCloseVoting = !liveGigSettings.acceptingVotes || isLastSong;
    
    if (shouldCloseVoting !== !liveGigSettings.votingOpen) {
      setLiveGigSettings(prev => ({...prev, votingOpen: !shouldCloseVoting}));
    }
    
    console.log('✅ Queue updated:', {
      total: limitedQueue.length,
      paid: limitedQueue.filter(s => s.isPaid).length,
      voted: limitedQueue.filter(s => s.voteCount > 0).length,
      votingOpen: !shouldCloseVoting,
      isLastSong
    });
    
    // Step 9: Update the queue
  ...
  const playSong = (song) => {
    ...
    // If autoplay is enabled, automatically move to next song after current song "finishes"
    if (liveGigSettings.autoPlayEnabled) {
      // Simulate song duration - in real app this would be actual song length
      const songDuration = 180000; // 3 minutes default
      setTimeout(() => {
        setLiveGigData(prev => ({...prev, currentSong: null}));
      }, songDuration);
    }
  };
  ...
  // Auto-check for live gigs in audience mode
  useEffect(() => {
    if (mode === 'aud' && !liveGig) {
      const checkForLiveGig = () => {
        const storedLiveGig = storage.get(STORAGE_KEYS.LIVE_GIG);
        if (storedLiveGig) {
          setLiveGig(storedLiveGig);
          console.log('✅ Auto-joined live gig!');
        }
      };
      ...
      const interval = setInterval(checkForLiveGig, 3000);
      
      return () => clearInterval(interval);
    }
  }, [mode, liveGig]);

  // Real-time sync between artist and audience tabs
  useEffect(() => {
    const syncInterval = setInterval(() => {
      if (liveGig) {
        console.log('🔄 Syncing from localStorage...');
        // Reload live gig data from localStorage (but keep current queuedSongs)
        const storedData = storage.get(STORAGE_KEYS.LIVE_GIG_DATA);
        if (storedData) {
          console.log('📥 Got data from storage:', {
            votes: storedData.votes,
            voteTimestamps: storedData.voteTimestamps,
            queueLength: storedData.queuedSongs?.length
          });
          setLiveGigData(prev => ({
            ...storedData,
            queuedSongs: prev.queuedSongs // Keep current queue, will be updated by updateGigQueue
          }));
        }
        const storedSettings = storage.get(STORAGE_KEYS.LIVE_GIG_SETTINGS);
        if (storedSettings) {
          setLiveGigSettings(storedSettings);
        }
      }
  ...
  // Update queue when votes or settings change - THIS IS KEY!
  useEffect(() => {
    if (liveGig) {
      console.log('🎯 Votes/settings changed, updating queue...', {
        votes: liveGigData.votes,
        timestamps: liveGigData.voteTimestamps,
        limit: liveGigSettings.songLimit
      });
      updateGigQueue();
    }
  }, [liveGigData.votes, liveGigData.voteTimestamps, liveGigData.songRequests, liveGigSettings.songLimit, liveGig]);

  // Auto-play functionality
  useEffect(() => {
    if (liveGig && mode === 'live' && liveGigSettings.autoPlayEnabled && !liveGigData.currentSong && liveGigData.queuedSongs.length > 0) {
      const timer = setTimeout(() => {
        playSong(liveGigData.queuedSongs[0]);
      }, liveGigSettings.autoPlayWaitTime * 1000);
      
      return () => clearTimeout(timer);
    }
  }, [liveGigData.currentSong, liveGigData.queuedSongs, liveGigSettings.autoPlayEnabled, liveGig, mode]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-900 to-indigo-900 p-4">
        ...
        <div className="max-w-6xl mx-auto">
          <header className="text-white mb-6 pt-6 flex justify-between items-center">
            ...
            <button onClick={()=>{setAuth(null);setAuthMode('');setMode('sel');}} className="px-6 py-3 bg-white/20 hover:bg-white/30 text-white rounded-lg font-bold">Exit</button>
          </header>
                          ...
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div className="flex-1">
                              <div className="text-white font-semibold flex items-center gap-2">
                                {song.title}
                                {isPaid && <span className="text-xs bg-yellow-500 text-black px-2 py-1 rounded font-bold">💰 ${song.paidAmount}</span>}
                                {voteCount > 0 && <span className="text-pink-300 font-bold text-sm">❤️ {voteCount}</span>}
                              </div>
              ...
              <div className="bg-white/10 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      onClick={()=>setLiveGigSettings({...liveGigSettings, acceptingVotes: !liveGigSettings.acceptingVotes, votingOpen: !liveGigSettings.acceptingVotes})}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-white font-semibold">🎵 Auto-Play</span>
                    <button 
                      onClick={()=>setLiveGigSettings({...liveGigSettings, autoPlayEnabled: !liveGigSettings.autoPlayEnabled})}
                      className={`px-4 py-2 rounded font-bold ${liveGigSettings.autoPlayEnabled ? 'bg-green-500' : 'bg-gray-500'}`}
                    >
                      {liveGigSettings.autoPlayEnabled ? 'ON' : 'OFF'}
                    </button>
                  </div>
                  {liveGigSettings.autoPlayEnabled && (
                    <div className="flex items-center justify-between pl-4">
                      <span className="text-white text-sm">Wait Time (seconds)</span>
                      <input 
                        type="number" 
                        value={liveGigSettings.autoPlayWaitTime}
                        onChange={(e)=>setLiveGigSettings({...liveGigSettings, autoPlayWaitTime: Math.max(5, Math.min(60, parseInt(e.target.value) || 15))})}
                        min="5"
                        max="60"
                        className="w-16 px-2 py-1 rounded bg-white/20 text-white border border-white/30 text-center"
                      />
                    </div>
                  )}
                </div>
          ...
          <div className="max-w-7xl mx-auto">
            <header className="text-white mb-6 pt-6 flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={()=>{setLiveGig(null);setMode('aud');}} className="px-6 py-3 bg-white/20 hover:bg-white/30 text-white rounded-lg font-bold">
                  Exit
                </button>
              </div>
            </header>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 to-green-900 p-4">
        <div className="max-w-4xl mx-auto">
          <header className="text-white mb-6 pt-6 flex justify-between">
            ...
            <button onClick={()=>{setAuth(null);setAuthMode('');setMode('sel');}} className="px-4 py-2 bg-white/20 text-white rounded-lg hover:bg-white/30">Exit</button>
          </header>
          ...
          <div className="bg-white/10 rounded-xl p-6 mb-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-teal-200">Automatically checking for live performances...</p>
            </div>
            <p className="text-gray-300 text-sm">When an artist goes live, you'll automatically join! 🎵</p>
          </div>
          
          <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 border-2 border-purple-400 rounded-xl p-6 mb-6">
            <h3 className="text-xl font-bold text-white mb-4">💡 How to Test</h3>
            <div className="space-y-3 text-purple-200">
              <p className="flex items-start gap-2">
                <span className="text-2xl">1️⃣</span>
                <span><strong>Open a new tab</strong> in the same browser (NOT incognito)</span>
              </p>
              <p className="flex items-start gap-2">
                <span className="text-2xl">2️⃣</span>
                <span><strong>Login as Artist</strong> in that tab</span>
              </p>
              <p className="flex items-start gap-2">
                <span className="text-2xl">3️⃣</span>
                <span><strong>Create a gig and click "Go Live"</strong></span>
              </p>
              <p className="flex items-start gap-2">
                <span className="text-2xl">4️⃣</span>
                <span><strong>Come back to this tab</strong> - you'll auto-join! ✨</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-blue-500/20 border border-blue-400 rounded-xl p-6">
            <p className="text-blue-200 mb-4">
              <strong>⚡ Manual Check:</strong> If you know an artist is live, click this button to join immediately!
            </p>
            <button 
              onClick={() => {
                const storedLiveGig = storage.get(STORAGE_KEYS.LIVE_GIG);
                if (storedLiveGig) {
                  ...
                  window.alert('⏳ No live gig found. Make sure an artist is live in another tab (same browser, NOT incognito).');
                }
              }}
              className="w-full px-6 py-4 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white rounded-lg font-bold shadow-lg text-lg"
            >
              🔍 Check for Live Gigs Now
            </button>
            
            <div className="mt-4 pt-4 border-t border-blue-400/30">
              <p className="text-yellow-200 text-sm">
                <strong>🔧 Troubleshooting:</strong> If the button doesn't work, make sure you're testing in the SAME browser (not incognito). Incognito has separate storage!
              </p>
            </div>
          </div>
        </div>
