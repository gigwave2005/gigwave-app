    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        try {
          ...
          // ✅ NEW: Check for active live gig FIRST
          console.log('🔍 Checking for active live gig...');
          const activeLiveGig = await getActiveLiveGigForArtist(user.uid);
          
          if (activeLiveGig) {
            console.log('🎸 ACTIVE LIVE GIG FOUND:', activeLiveGig.venueName);
            
            // Load the live gig data
            setLiveGig(activeLiveGig);
            setMode('live');
            
            // Save to localStorage
            localStorage.setItem(`GigWave_live_${user.uid}`, JSON.stringify(activeLiveGig));
            localStorage.setItem('GigWave_mode', 'live');
            
            console.log('✅ Artist locked into live mode');
            
          } else {
            console.log('✅ No active live gig found');
...
useEffect(() => {
  ...
  const unsubscribe = listenToLiveGig(liveGig.id, async (gigData) => {
    ...
    // ✅ FIXED: Added songRequests and other missing fields
    setLiveGigData({
      votes: gigData.votes || {},
      ...
      audienceTracking: gigData.audienceTracking || {  // ✅ ADD THIS
        totalJoins: 0,
        currentlyActive: 0,
        joinedUsers: {}
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-teal-900 to-blue-900 p-4">
        <div className="max-w-6xl mx-auto pt-6">
          ...
          {/* Live Mode Lock Warning */}
          <div className="bg-orange-500/20 border-2 border-orange-400 rounded-lg p-4 mb-6">
            <p className="text-orange-300 font-bold text-center">
              🔒 You are locked in LIVE mode. You must end this gig to access other features.
            </p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {/* Audience Stats */}
            <div className="bg-pink-500/20 border border-pink-400 rounded-xl p-6 text-center">
              <div className="text-4xl font-bold text-white mb-2">
                {liveGigData.audienceTracking?.currentlyActive || 0}
              </div>
              <div className="text-pink-200 font-semibold text-sm">👀 Live Now</div>
            </div>
            <div className="bg-magenta/20 border border-magenta rounded-xl p-6 text-center">
              <div className="text-4xl font-bold text-white mb-2">
                {liveGigData.audienceTracking?.totalJoins || 0}
              </div>
              <div className="text-magenta font-semibold text-sm">🎫 Total Joins</div>
            </div>
            ...
            <div className="bg-green-500/20 border border-green-400 rounded-xl p-6 text-center">
              ...
              <div className="text-green-200 font-semibold text-sm">⚡ Total Votes</div>
            </div>
            
            {/* ✅ NEW: Pending Requests */}
            <div className="bg-orange-500/20 border border-orange-400 rounded-xl p-6 text-center">
              <div className="text-4xl font-bold text-white mb-2">
                {(liveGigData.songRequests || []).filter(r => r.status === 'pending').length}
              </div>
              <div className="text-orange-200 font-semibold text-sm">🎤 Pending Requests</div>
            </div>
            ...
            <div className="bg-blue-500/20 border border-blue-400 rounded-xl p-6 text-center">
              ...
              <div className="text-blue-200 font-semibold text-sm">💬 Comments</div>
            </div>
            */}
            
            {/* ❌ COMMENTED OUT - Donations */}
            {/* 
            <div className="bg-purple-500/20 border border-purple-400 rounded-xl p-6 text-center">
              ...
              <div className="text-purple-200 font-semibold text-sm">💰 Donations</div>
            </div>
            */}
          </div>
                            ...
                            {request.status === 'pending' && (
                              <>
                                <button
                                  onClick={async () => {
                                    try {
                                      console.log('🎵 Accepting song request:', request.songTitle);
                                      ...
                                      // ✅ Count only unplayed songs in queue
                                      const activeQueue = currentQueue.filter(s => !playedSongs.includes(s.id));
                                      ...
                                      // ✅ NEW LOGIC: If queue is full, remove last song (lowest priority)
                                      if (activeQueue.length >= maxQueueSize) {
                                        const removedSong = activeQueue[activeQueue.length - 1];
                                        messageExtra = `\n\n⚠️ Queue was full (${maxQueueSize} songs).\n\nRemoved: "${removedSong.title}" by ${removedSong.artist}`;
                                        
                                        console.log('🗑️ Removing last song to make room:', removedSong.title);
                                        
                                        // Remove last song from queue
                                        const updatedQueue = currentQueue.filter(s => s.id !== removedSong.id);
                                        await updateDoc(gigRef, {
                                          queuedSongs: updatedQueue
                                        });
                                        
                                        console.log('✅ Last song removed, queue now has:', updatedQueue.length, 'songs');
                                      }
                                      
                                      // ✅ Accept the request (will add to queue)
                                      await acceptSongRequest(liveGig.id, request.id);
                                      
                                      console.log('✅ Request accepted and added to queue');
                                      
                                      alert(`✅ Request accepted!${messageExtra}`);
                                    } catch (error) {
        ...
        {showVoteModal && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="rock-background gig-card border-2 border-magenta max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="sticky top-0 bg-black/95 p-6 border-b border-white/10 z-10">
                <div className="flex justify-between items-center">
                  <h2 className="concert-heading text-3xl text-magenta">⚡ VOTE FOR SONGS</h2>
                  <button
        ...
        {showArtistProfileModal && artistProfile && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="rock-background gig-card border-2 border-electric max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="sticky top-0 bg-black/95 p-6 border-b border-white/10 z-10">
                <div className="flex justify-between items-center">
                  <h2 className="concert-heading text-3xl text-electric">🎸 ARTIST PROFILE</h2>
                  <button
              ...
              <div className="p-6">
                ...
                <div className="text-center mb-6">
                  <h1 className="concert-heading text-4xl text-white mb-3">
                ...
                {/* Bio */}
                <div className="mb-6">
                  <h3 className="text-xl font-bold text-electric mb-3">About</h3>
                  <p className="text-gray-light leading-relaxed">{artistProfile.bio}</p>
                </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="gig-card max-w-2xl w-full max-h-[90vh] overflow-y-auto border-2 border-magenta">
              ...
              <div className="mb-6 p-4 bg-blue-500/20 border border-blue-400 rounded-lg">
                ...
                {/* ✅ NEW: Always show positive message */}
                <div className="mt-3">
                  {(() => {
                  ...
                  {liveGig.masterPlaylist && liveGig.masterPlaylist.length > 0 ? (
                    liveGig.masterPlaylist
                      .filter(song => {
                        ...
                        // ✅ NEW: Filter out songs already in gig playlist (queuedSongs)
                        const isInGigPlaylist = (liveGigData.queuedSongs || []).some(
                          queuedSong => queuedSong.id === song.id
                        );
                        
                        return !isInGigPlaylist; // Only show songs NOT in gig playlist
                      })
                      .map(song => (
                        <div
                          key={song.id}
              ...
              <div className="flex flex-col md:flex-row gap-3">
                <button
                  ...
                  className={`flex-1 py-4 rounded-lg font-bold text-lg transition ${
                    selectedRequestSong
                      ? 'btn btn-neon' // ✅ CHANGED FROM btn-fire to btn-neon
