...
export default function App() {
  ...
  useEffect(() => {
    const syncInterval = setInterval(() => {
      if (liveGig) {
        // Reload live gig data from localStorage
        const storedData = storage.get(STORAGE_KEYS.LIVE_GIG_DATA);
        if (storedData) {
          setLiveGigData(storedData);
        }
        ...
        // Trigger queue update after syncing data
        if (mode === 'live') {
          setTimeout(() => updateGigQueue(), 100);
        }
      }
  ...
  // Update queue when votes or settings change
  useEffect(() => {
    if (liveGig && (mode === 'live' || mode === 'aud')) {
      updateGigQueue();
    }
  }, [liveGigData.votes, liveGigData.songRequests, liveGig]);
