...
export default function App() {
  ...
  const updateGigQueue = () => {
    ...
    // Get paid requests (always priority)
    const paidRequests = liveGigData.songRequests
      .filter(r => r.status === 'pending')
      .map(r => ({...r.song, isPaid: true, paidAmount: r.amount}));
    
    // Get all voted songs from master playlist with their vote counts
    const votedSongs = Object.entries(liveGigData.votes || {})
      .filter(([id, count]) => count > 0)
      .map(([id, count]) => {
        const song = masterSongs.find(s => s.id === parseInt(id));
        return song ? {
          ...song,
          voteCount: count,
          voteTimestamp: liveGigData.voteTimestamps?.[id] || Date.now()
        } : null;
      })
      .filter(Boolean)
      .sort((a, b) => {
        // Sort by vote count descending, then by timestamp ascending (earlier votes first)
    ...
    // Combine: Paid requests first, then voted songs, then playlist songs
    ...
    // Check if voting should be closed (queue is full with voted songs)
    const votedSongsInQueue = limitedQueue.filter(s => s.voteCount > 0).length;
    const shouldCloseVoting = votedSongsInQueue >= liveGigSettings.songLimit;
